...
def data_loader_mnist(dataset):
    # This function reads the MNIST data and separates it into train, val, and test set
    ...
    Xtest = test_set[0]
    Ytest = test_set[1]
    return np.array(Xtrain), np.array(Ytrain), np.array(Xvalid),np.array(Yvalid), np.array(Xtest), np.array(Ytest)
...
def main(main_params):
    ...
    """
    Building MLP for a 10-class classification problem on MNIST
    ...
    # creating objects from the module classes
    ...
    # creating variables for momentum
    if _alpha > 0.0:
