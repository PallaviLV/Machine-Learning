...
def data_loader_mnist(dataset):
    ...
    return np.array(Xtrain).reshape(-1, 1, 28, 28), np.array(Ytrain), np.array(Xvalid).reshape(-1, 1, 28, 28),np.array(Yvalid), np.array(Xtest).reshape(-1, 1, 28, 28), np.array(Ytest)
...
def main(main_params):
    ...
    # building a CNN for 10-class classification problem on MNIST
    """
    ...
    # create objects from the module classes
